...
//            return GV.hintTable.count
    ...
    let NormalButtonCount: CGFloat = 3
    var countButtons: CGFloat = 0

    public func resetDeveloperButton() {
        if countButtons == NormalButtonCount {
            return
        }
    ...
    public func generateDebugButton() {
        countButtons = NormalButtonCount + 1
...
//                print("In Update: countFoundedWords: \(AW.addingWordData.countFoundedWords), lastWord: \(AW.addingWordData.lastWord), cellIndexes: \(AW.addingWordData.callIndexesLeft)")
    ...
    @objc private func stopSearching() {
        fixWordsHeader.text = GV.language.getText(.tcFixWords, values: "0", "0")
...
//        gameLayer.position = CGPoint(x: GV.actWidth * 0.5, y: GV.actHeight * 0.5)
    ...
    private func setButtonsPositions() {
        let firstPosXP = (GV.minSide / countButtons) / 2
        let firstPosXL = (GV.maxSide / countButtons) / 2
        let xAdderP = GV.minSide / countButtons
        let xAdderL = GV.maxSide / countButtons
...
//        chooseSizeButton.setActPosSize()
    ...
    private func addButtonPL(to: SKNode, text: String, action: Selector, buttonType: ButtonType)->MyButton {
        ...
        let firstPosX = (GV.actWidth / countButtons) / 2
        let xAdder = GV.actWidth / countButtons
        switch buttonType {
//        case .SizeButton:
...
//    var demoModus: DemoModus = .Normal
    ...
    private func myTouchesMoved(touchLocation: CGPoint) {
        ...
        if OK {
            ...
            let delta = touchLocation.x - actPosition.x
            if delta > 0 {
...
//        chooseSizeButton.isHidden = true
    ...
    private func makeTextBox()->SKSpriteNode {
        ...
        let size = CGSize(width: maxWidth * 1.05, height: textHeight * CGFloat(textFragments.count))
        let node = SKSpriteNode(texture:  DrawImages.drawBoxWithRoundedEdges(size:size))
//        shape.path = UIBezierPath(roundedRect: CGRect(x: GV.minSide * 0.5, y: GV.maxSide * 0.5, width: maxWidth * 1.05, height: textHeight * CGFloat(textFragments.count)), cornerRadius: 7.5).cgPath
//        node.fillColor = UIColor.yellow
        ...
        for text in textFragments {
            ...
            textLabel.position = CGPoint(x: node.frame.midX, y: node.frame.maxY - (1 + line) * textHeight * 0.8)
...
//        self.scene?.alpha = 0.2
    ...
    private func congratulation() {
        ...
        let myAlert = MyAlertController(title: GV.language.getText(.tcCongratulations),
                                        message: GV.language.getText(.tcFinishGameMessage, values: String(getCountWords(type: .CountAllWords)), String(getCountWords(type: .CountNewWords)), String(score)),
...
//    var showDemoNextTimeButton: MyButton!
    ...
    let fontSize: CGFloat = GV.onIpad ? 22 : 15
    let MovingLayerName = "MovingLayer"
    var difficultyNameArray = [String]()
    
    private func setDifficultyNameArray() {
        difficultyNameArray = [GV.language.getText(.tcEasy), GV.language.getText(.tcMedium), GV.language.getText(.tcHard)]
    }
    ...
    public func playingGame() {
        ...
        if origGames.count > 0 {
            ...
            let text = GV.language.getText(.tcFixWords, values: String(getCountWords(type: .CountMandatoryWords)), String(playedGame.wordsToFind.count))
            let fixWordsHeaderPosition = PLPosSize(PPos: CGPoint(x: GV.minSide * 0.5, y: gridPosition.PPos.y - GV.playingGrid!.plPosSize!.PSize!.height * 0.55),
                                                   LPos: CGPoint(x: GV.maxSide * 0.18, y: gameHeaderPosition.LPos.y))
            fixWordsHeader = MyLabel(text: text, position: fixWordsHeaderPosition, fontName: GV.headerFontName, fontSize: fontSize)
...
//                movingLayer.color = UIColor(red: 1, green: 0, blue: 0, alpha: 0.8)
                self.gameLayer.addChild(movingLayer)
            }

            tippButton = addButtonPL(to: gameLayer, text: GV.language.getText(.tcTipp), action: #selector(showTipp), buttonType: .TippButton)
            myWordsButton = addButtonPL(to: gameLayer, text: GV.language.getText(.tcShowMyWords, values: String(getCountWords(type: .CountAllWords))), action: #selector(showMyWords), buttonType: .WordsButton)
            if GV.developerModus && developerButton == nil {
                generateDebugButton()
            }
...
//                        myLabels[itemIndex].usedWord = choosedWord
    ...
    enum CounterType: Int {
        case CountAllWords = 0, CountNewWords, CountMandatoryWords
    }
    private func getCountWords(type: CounterType)->Int {
        ...
        switch type {
        case .CountAllWords: return countAllWords
        case .CountNewWords: return countNewWords
        case .CountMandatoryWords: return countFoundedMandatoryWords
        }
    }
