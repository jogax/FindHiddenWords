//
//  en.swift
//  DuelOfWords
//
//  Created by Romhanyi Jozsef on 2020. 05. 09..
//  Copyright © 2020. Romhanyi Jozsef. All rights reserved.
//



let enDictionary: [TextConstants: String] = [
    .tcEmpty:                   "",
    .tcAktLanguage:             "en",
    .tcLanguage:                "Language",
    .tcEnglish:                 "English (English)",
    .tcGerman:                  "German (Deutsch)",
    .tcHungarian:               "Hungarian (Magyar)",
    .tcRussian:                 "Russian (Русский)",
    .tcEnglishShort:            "en",
    .tcGermanShort:             "de",
    .tcHungarianShort:          "hu",
    .tcRussianShort:            "ru",
    .tcMainTitle:               "Settings",
    .tcStartGame:              "Play",
    .tcChooseLanguage:          "Show languages",
    .tcGameMenuTitle:           "Choose a game",
    .tcChooseLanguageTitle:     "Choose language",
    .tcDuelOfWords:             "Duel of Words",
    .tcSearchWords:             "Search Words (%)",
    .tcCubeOfWords:             "Cube of Words",
    .tcBack:                    "Return",
    .tcDeveloperMenu:           "Developer Options",
    .tcGenerateGameArray:       "Generate Game Array",
    .tcDeveloperMenuTitle:      "Choose an action",
    .tcGeneratingSize:          "Size of Game Array: %",
    .tcSizeMenuTitle:           "Choose a size",
//    .tcSearchingWords:          "Searching Words",
    .tcGameSize:                "Gamesize: %",
    .tcGameNumber:              "#%",
    .tcGameHeader:              "Game: #%",
    .tcFixWords:                "Found words: (%/%)",
    .tcPlayGame:                 "New game",
    .tcFinishedGame:            "Finished Games",
    .tcAlphabet:                "ABCDEFGHIJKLMNOPQRSTUVWXYZ",
    .tcCongratulations:         "Congratulations!",
    .tcFinishGameMessage:       "You have finished the Game. ° You found % words ° (% new) ° and got % points! ° Now starts a new game.",
    .tcOK:                      "OK",
    .tcMyScore:                 "My score: %",
    .tcWorldBestScore:          "1. place: % - %",
    .tcDeviceBestScore:         "My best score: %",
    .tcShowMyWords:             "Words (%)",
    .tcWord:                    "Word:",
    .tcCount:                   "Cnt:",
    .tcLength:                  "Len:",
    .tcScoreTxt:                "Score:",
    .tcCollectedOwnWords:       "My Words",
    .tcWordsOverLetter:         "Words over the letter: \"%\"",
    .tcChooseSize:              "Gamesize",
    .tcBlank:                   " ",
    .tcPlayer:                  "Player",
    .tcDevice:                  "Device",
    .tcVersion:                 "Version",
    .tcLand:                    " Land",
    .tcDeveloper:               "Developer",
    .tcShowGameCenter:          "Show GameCenter",
    .tcOnlineTime:              "Online   ",
    .tcLastOnline:              "Last           ",
    .tcLastOnlineTime:          "Duration",
    .tcGameCenter:              "GameCenter ViewController",
    .tcSearchingMoreWords:      "Search new words",
    .tcStopSearching:           "Stop searching",
//    .tcDisable:                 "Disable DeveloperButton",
    .tcStopDemo:                "Stpp Demo",
    .tcShowDemoLater:           "Show later",
    .tcSettings:                "Settings",
    .tcTipp:                    "Tip",
    .tcChooseShowingType:       "Show fouded letters or dots",
    .tcEasy:                    "Easy",
    .tcMedium:                  "Medium",
    .tcHard:                    "Hard",
    .tcChooseDifficulty:        "Difficulty",
    .tcHowToMoveWords:          "By moving the word list°to the left or to the right,°you can see all the words°you need to find.",
]
